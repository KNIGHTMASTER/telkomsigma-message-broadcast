 ...
 */
@Component
public class BasicEmailQueueProducer implements IJMSProducer<BasicMailRequestDTO> {

    @Autowired
    private JmsMessagingTemplate jmsMessagingTemplate;

    @Autowired
    @Qualifier(AppConstant.Messaging.Queue.EMAIL)
    private Queue queueBasicMail;

    @Override
    public void send(BasicMailRequestDTO p_Message) {
        this.jmsMessagingTemplate.convertAndSend(queueBasicMail, p_Message);
    }
}
